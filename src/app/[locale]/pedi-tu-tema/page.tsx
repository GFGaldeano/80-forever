...
type LocalizedSongRequestPageProps = {
  params: Promise<{
    locale: string;
  }>;
};
...
export async function generateMetadata({
  params,
}: Readonly<LocalizedSongRequestPageProps>): Promise<Metadata> {
  ...
  const title = `${dictionary.songRequestPage.title} | ${seo.siteName}`;
  const description = dictionary.songRequestPage.description;
  const canonical = buildAbsoluteSiteUrl(seo.siteUrl, `/${locale}/pedi-tu-tema`);
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: {
        es: buildAbsoluteSiteUrl(seo.siteUrl, "/es/pedi-tu-tema"),
        en: buildAbsoluteSiteUrl(seo.siteUrl, "/en/pedi-tu-tema"),
      },
    },
    openGraph: {
      title,
...
export default async function LocalizedSongRequestPage({
  params,
}: Readonly<LocalizedSongRequestPageProps>) {
  const { locale } = await params;
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-12 md:px-8">
        <div className="border-b border-white/10 pb-8 text-center">
          ...
          <div className="mt-4 inline-flex items-center gap-3 rounded-full border border-fuchsia-500/20 bg-fuchsia-500/10 px-4 py-2 text-fuchsia-300">
            ...
            {dictionary.songRequestPage.badge}
          </div>

          <h1 className="mt-6 text-4xl font-semibold tracking-tight md:text-5xl">
            {dictionary.songRequestPage.title}
          </h1>

          <p className="mx-auto mt-4 max-w-3xl text-sm leading-7 text-zinc-400 md:text-base">
            {dictionary.songRequestPage.description}
          </p>
        </div>
